...
class Obstacle:
    def __init__(self, x, y, type):
        self.x, self.y = x, y
        if type == 1:
            self.minx, self.maxx = x - 70 , x + 90
        else:
            self.minx, self.maxx = x - 70, x + 110
...
def enter():
    global mouse, background, end, bar_1, bar_2, bar_3, bar_4, bar_5, bar_6, bar_7, bar_8, bar_9, bar_10, bar_11, bar_12, obstacle, obstacle2, obstacle3
    global drawstart
    drawstart = 0
    mouse = Mouse()
    end = End()
    ...
    obstacle3 = Obstacle(230, 220, 1)
    mouse.setpos()

def exit():
    global mouse, background, end, bar_1, bar_2, obstacle, obstacle2, obstacle3
    del(mouse)
    del(background)
    del(end)
    del(bar_1)
...
def handle_events():
    ...
    for event in events:
        if event.type == SDL_QUIT:
            ...
            game_framework.push_state(map4)
        else :
